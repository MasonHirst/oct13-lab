...
// CODE HERE
function addBackgroundBlack() {
    sayHelloButton.classList.add('background-color')
}

sayHelloButton.addEventListener('mouseover', addBackgroundBlack)
...
// CODE HERE
function resetStyle() {
    sayHelloButton.classList.remove('background-color')
}

sayHelloButton.addEventListener('mouseout', resetStyle)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/' + 'Mason')
    .then((response) => {
...
function customQuery() {
    axios.get('http://localhost:3000/query-test/?name=mason/?height=45')
    .then((response) => {
        console.log(response)
    }).catch()
}

document.querySelector('#query-button').addEventListener('click', customQuery)
...
/*
    ...
    2: Send more than 1 query on the URL -- what happened? 
        //It said 'You sent query: name with value: mason/?height=45!'. so it didn't really register the 2nd query.
*/
...
document.querySelector('#food-add-button').addEventListener('click', createFood)
